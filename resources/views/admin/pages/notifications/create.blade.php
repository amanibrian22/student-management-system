...
<head>
    ...
    <title>Add Announcement - AcademiTrack</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
            <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-100">
                <h1 class="text-2xl font-bold text-gray-800 mb-6">Add Announcement</h1>
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <form action="{{ route('announcements.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label for="title" class="block text-sm text-gray-700 font-semibold mb-2">Title</label>
                            <input type="text" name="title" id="title" class="w-full p-2 border rounded-lg text-sm" value="{{ old('title') }}" required>
                            @error('title')
                        ...
                        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 text-sm">Create Announcement</button>
