...
<head>
    ...
    <title>Edit Attendance - AcademiTrack</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
            <main class="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-100">
                <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Attendance</h1>
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <form action="{{ route('attendances.update', $attendance) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="student_id" id="student_id" class="w-full p-2 border rounded-lg text-sm" required>
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ $student->id == $attendance->student_id ? 'selected' : '' }}>{{ $student->name }} ({{ $student->student_id }})</option>
                                @endforeach
                            </select>
                            @error('student_id')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="date" id="date" class="w-full p-2 border rounded-lg text-sm" value="{{ $attendance->date }}" required>
                            @error('date')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="subject" id="subject" class="w-full p-2 border rounded-lg text-sm" value="{{ $attendance->subject }}" maxlength="10" required>
                            @error('subject')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        </div>
                        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm">Update Attendance</button>
                    </form>
                </div>
            </main>
        </div>
